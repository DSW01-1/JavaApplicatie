	...
	private int findNext(int index)
	{
		...
		for (GridTile tile : CoordList)
		{
			if (!usedIndex.contains(cnt) && cnt != index)
			{
				...
				simulatie.addConsoleItem(String.format("  Tile %s X = %s, Y = %s", index + 1,
						CoordList.get(index).getXcoord(), CoordList.get(index).getYcoord()), "DEBUG");
				simulatie.addConsoleItem(
						String.format("  Tile %s X = %s, Y = %s", cnt + 1, tile.getXcoord(), tile.getYcoord()),
						"DEBUG");
				simulatie.addConsoleItem(
						String.format("  Difference between Tile %s and Tile %s: X = %s, Y = %s, Combined = %s",
								index + 1, cnt + 1, xDiff, yDiff, xyDiff),
						"DEBUG");
