...
public class DecreasingFirstFit {
    ...
    //initialize currentBox with volume 0 so a new one gets called at the start.
    ...
    public ArrayList<Box> executeDecreasingFirstFit(ArrayList<Product> products){
        ...
        for (Product product : products) {
            ...
            if (fitted == false) {
                ...
                simulation.addConsoleItem("Product did not fit with size: "+product.GetSize()+", new box created and added, amount of boxes "+returnBoxes.size(),"DEBUG");
            }
            fitted = false;
        }
